...
Make a directory for the install and work in it. Mint this instance's secrets —
they are generated by the same image you are about to run:

```bash
docker run --rm ghcr.io/galgtonold/sojourn:latest node scripts/selfhost-init.mjs --stdout > .env
```
...
Fetch the one file that describes the stack, **naming it `docker-compose.yml`**:

```bash
curl -fLo docker-compose.yml https://github.com/galgtonold/sojourn/releases/latest/download/docker-compose.all-in-one.yml
```

Open `.env` and set `SUPABASE_PUBLIC_URL` and `SITE_URL` for your host, then:

```bash
docker compose up -d
```

> **Why those two names.** Compose looks for `docker-compose.yml` and reads
> `.env` without being told, so there are no `-f` or `--env-file` flags — here
> or, more importantly, when you update. The admin's Updates page prints the
> short command, and an operator should not have to reconstruct it from two
> filenames they chose months earlier.
>
> An existing install using `docker-compose.all-in-one.yml` and `.env.selfhost`
...
> it in `.env` to choose how much change you take unattended:
> `0.2.1`, `0.2`, `0`, or `latest`.
...
**Keep `.env` with your backups.** It is deliberately not inside the
